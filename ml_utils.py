...
def fully_connected_layer(input, output_dim, activation_type='Sigmoid', input_dropout=1):
  input_dim = input.shape[-1]
  with tf.name_scope('Layer'):
    W = tf.Variable(tf.random_normal([input_dim, output_dim], 0.0, 0.1), name='W1')
